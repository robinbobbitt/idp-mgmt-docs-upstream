...
- `ldap` contains the LDAP specific configurations:
  ...
  * `bindDN` contains DN for an application service account. The connector uses the bindDN and bindPW as credentials to search for users and groups. Not required if the LDAP server provides access for anonymous auth.
  * `bindPassword` contains reference to a secret by name containing a password to bind with during the search phase.
