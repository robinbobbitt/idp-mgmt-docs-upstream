...
. *Note:* For bare metal only, you need to provide the certificate information for the disconnected registry in your `install-config.yaml` file. To access the image in a protected disconnected registry, you must provide the certificate information so the identity configuration management for Kubernetes operator can access the registry.
...
//TODO Revisit this when we have official mirror name
...
spec:
  ...
  - mirrors:
    - mirror.registry.com:5000/idp-mgmt-config
    source: registry.redhat.io/idp-mgmt-config
----
